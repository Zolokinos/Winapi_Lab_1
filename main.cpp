...
HMENU NewMainMenu() {
    ...
    AppendMenu(hMenuBar, MF_POPUP, IDM_INFO, "&Info");
...
//            ReleaseDC(window_handle,hdc);
            ...
            hMainMenu = NewMainMenu();
            SetMenu(window_handle, hMainMenu);
            hStatusBar = CreateWindow(
          STATUSCLASSNAME, NULL, WS_CHILD | WS_VISIBLE,
          0, 0, 0, 0, window_handle, (HMENU) 1, GetModuleHandle(NULL), NULL);

            SendMessage(hStatusBar, SB_SETTEXT, NULL,
                        (LPARAM) "Line Mode");
            break;
        }
        ...
        case WM_SIZE: {
            ...
            MoveWindow(hStatusBar, 0, 0, -1, -1, true);
            break;
        }
        case WM_LBUTTONDOWN: {
            if (w_param & MK_SHIFT) {
                ...
                switch (LOWORD(w_param)) {
                    ...
                    case IDM_INFO: {
                        MessageBox(window_handle, (
                                "r- rectangles, p- polygons \r\n"
                                "l- line segment, b- broken line \r\n"
                                "e- ellipses, c- circles, s- sectors"), "Info", MB_OK);
                        break;
                    }
                    case IDM_QUIT: {
                        PostQuitMessage(0);
